...
LOGGER = flask.logging.create_logger(storm_platform.app)


@storm_platform.app.route('/accounts/login/')
def show_login():
...
def delete_file(file_dict):
    ...
    path = storm_platform.app.config["UPLOAD_FOLDER"]/filename
...
def compute_basename(fileobj):
    ...
    # Save to disk
    path = storm_platform.app.config["UPLOAD_FOLDER"]/uuid_basename
    fileobj.save(path)
...
@storm_platform.app.route('/accounts/', methods=['POST'])
def login():
...
@storm_platform.app.route('/accounts/logout/', methods=['POST'])
def logout():
